...
class ImageProcessing {
    ...
    fun processImage(img: Bitmap, resizeImgCoefficient: Int = 1): Bitmap =
        processImageInner(img, resizeImgCoefficient)

    fun setEggParamListener(listener: (params: EggParameters) -> Unit) {
        eggParamsListener = listener
    }

    private fun processImageInner(value: Bitmap, resizeImgK: Int): Bitmap {
        val value1 = value.scale(value.width / resizeImgK, value.height / resizeImgK, false)
        val mat = Mat()
        val rgbImgMat = Mat()

        Utils.bitmapToMat(value1, mat)
        Utils.bitmapToMat(value1, rgbImgMat)

        Imgproc.cvtColor(mat, mat, Imgproc.COLOR_BGR2GRAY)
        Imgproc.threshold(mat, mat, 40.0, 255.0, Imgproc.THRESH_BINARY)
        // Adaptive threshold not work correct
//        Imgproc.adaptiveThreshold(mat, mat, 250.0, Imgproc.ADAPTIVE_THRESH_MEAN_C, Imgproc.THRESH_BINARY, 11, 12.0)

        val cnts = mutableListOf<MatOfPoint>()
        Imgproc.findContours(mat, cnts, Mat(), Imgproc.RETR_TREE, Imgproc.CHAIN_APPROX_SIMPLE)
        Imgproc.drawContours(rgbImgMat, cnts, -1, Scalar(255.0, 0.0, 0.0), 4)
